...
const ProductClientListDataGrid: FC<{ clientId?: string }> = ({ clientId }) => {
  const { enqueueSnackbar } = useSnackbar();

  const callProduct = useMemo(() => isString(clientId), [clientId])

  const openAddCampaignProductDialog = useBoolean();

  const { getClientProducts, loading: mainLoading, products: mainProducts } = useProducts(callProduct);
  ...
  const productsLoading = useMemo(() => {
    ...
    return mainLoading
  }, [mainLoading, loadingReport.value, clientId])
...
export default memo(ProductClientListDataGrid)