...
export default function UserStockDataGrid({
  campaign,
  ...
  const [selectedCampaignUsers, setSelectedCampaignUsers] = useState<
    ICampaignStockGridRow[] | null
  >(null);
  ...
  useEffect(() => {
    if (isString(campaignId) && !isEmpty(campaignId) && snapshotDateTime && !loadingStock.value) {
      ...
      realmApp.currentUser?.functions
        .getCampaignStock(campaignId.toString(), snapshotDateTime.toISOString())
  ...
  useEffect(() => {
    if (isString(campaignId) && !isEmpty(campaignId) && snapshotDateTime) {
      ...
      realmApp.currentUser?.functions
        .getCampaignStockMetrics(campaignId.toString(), snapshotDateTime.toISOString())
  ...
  const handleBulkAssignProducts = useCallback(
    (cmpUsers: ICampaignStockGridRow[]) => {
      if (cmpUsers) {
  ...
  const totalStock = useMemo(
    () =>
      Array.isArray(productRank)
        ? productRank.reduce(
            (acc, item) => acc + (isNumber(item.totalStock) ? item.totalStock : 0),
            0
          )
        : 0,
    [productRank]
  ...
  const totalSold = useMemo(
    () =>
      Array.isArray(productRank)
        ? productRank.reduce(
            (acc, item) => acc + (isNumber(item.totalSold) ? item.totalSold : 0),
            0
          )
        : 0,
    [productRank]
  ...
  const topProduct = useMemo(
    () => (Array.isArray(productRank) ? productRank[0] : null),
    [productRank]
  );
          ...
          <Grid container spacing={3}>
            <Grid xs={12} display="flex" justifyContent="space-evenly">
              ...
              >
                ...
                <LoadingButton
                  type="submit"
                  loading={isSubmitting || showLoader || showCampaignKPIMetricsLoader}
                  color="success"
                  variant="contained"
                >
                  Get Snapshot
            ...
            <DataGridFlexible
              ...
              customActions={{
                routes: {
                  ...
                  action: (selectedData: ICampaignStockGridRow[]) =>
