...
import { Card, Stack, Button, styled, useTheme, ButtonBase } from '@mui/material';
...
export default function UserActivityDataGrid({
  campaign,
  handleOpenCheckInRouteView,
}: IUserActivityDataGridProps) {
  ...
  const [campaignKPIMetrics, setCampaignKPIMetrics] = useState<ICampaignKPIMetricsResponse | null>(
    null
  );
  ...
  const NewCurrectSchema = Yup.object().shape({
    snapshotDateTime: Yup.date().transform((value, originalValue) => {
  ...
  const showLoader = useShowLoader(loadingCampaignUsers.value || loadingStock.value, 500);
  ...
  useEffect(() => {
    if (isString(campaignId) && !isEmpty(campaignId) && snapshotDateTime) {
      ...
      realmApp.currentUser?.functions
        .getCampaignUsers(campaignId.toString(), snapshotDateTime.toISOString())
  ...
  useEffect(() => {
    if (isString(campaignId) && !isEmpty(campaignId) && snapshotDateTime && !loadingStock.value) {
      campaignKPIMetricsLoading.onTrue();
      setCampaignUsersError(null);
      realmApp.currentUser?.functions
        .getCampaignKPIMetrics(campaignId.toString(), snapshotDateTime.toISOString())
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [campaignId, snapshotDateTime, loadingStock.value]);
  ...
  const handleAssignProductsToUser = useCallback(
    (ids: string[]) => {
      const sUsers = campaignUsers.filter((campaignUser) =>
        ids.some((x) => x.toString() === campaignUser._id.toString())
      );
      if (sUsers) {
  ...
  const handleEditRow = useCallback(
    (id: string) => {
      const user = campaignUsers.find(
        (campaignUser) => campaignUser._id.toString() === id.toString()
      );
      if (user && handleOpenCheckInRouteView) {
        handleOpenCheckInRouteView(user);
      }
  ...
  const totalLiveUsers = useMemo(
    () => cleanedUsers.filter((x) => x.isCheckedIn).length,
    [cleanedUsers]
  );
          ...
          <Grid container spacing={3}>
            <Grid xs={12} display="flex" justifyContent="space-evenly">
              <Stack>
                ...
                <StyledLabel>
                  {changedSnapshot
                    ? formatDifference(new Date(changedSnapshot))
                    : formatDifference(new Date())}
                </StyledLabel>
              </Stack>
              <Stack
                direction="row"
              ...
              >
                ...
                <LoadingButton
                  type="submit"
                  loading={isSubmitting || showActivityLoader || showCampaignKPIMetricsLoader}
                  color="success"
                  variant="contained"
                >
                  Get Snapshot
            ...
            <DataGridFlexible
              ...
              customActions={{
                routes: {
                  ...
                  action: (selectedData: ICampaignUser[]) =>
                    console.log(selectedData, 'USERS TO ASSIGN ROUTES'),
                },
                products: {
                  label: 'Assign Product',
                  color: 'info',
