  ...
  useEffect(() => {
    if (client?._id) {
      ...
      realmApp.currentUser?.functions
        .getClientUsers(client?._id.toString())
        .then((usrs: IUser[]) => {
          setUsers(
            usrs
              .map((x) => ({ ...x, _id: x._id.toString() }))
              .filter((z) => z.role === ERole.PROJECT_MANAGER)
          );
        })
        .catch((e) => {
          console.error(e);
  ...
  const handleChangeTab = useCallback(
    (event: React.SyntheticEvent, newValue: string) => {
      ...
      const href = currentProject
        ? `${paths.v2.client.project.edit(currentProject._id.toString())}?${tabParams}`
        : `${paths.v2.client.project.new}?${tabParams}`;
      router.push(href);
    },
    ...
    >
      <Stack direction="row" spacing={3} alignItems="center">
        ...
        <Typography textTransform="uppercase" color="text.secondary">
          {currentProject?.title ?? 'New Campaign'}
        </Typography>
      </Stack>
