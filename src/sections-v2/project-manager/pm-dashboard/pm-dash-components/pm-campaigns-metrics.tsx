...
function PMDashboardCampaignMetrics() {
  ...
  useEffect(() => {
    if (client?._id) {
      ...
      setError(null);
      realmApp.currentUser?.functions.getPMDashboardMetrics(client?._id.toString())
  ...
  useEffect(() => {
    if (client?._id) {
      ...
      realmApp.currentUser?.functions.getPMDashboardMetricsUpdated(client?._id.toString()).then((data: IClientDashboardSummaryUpdated) => setDashboardMetricsUpdated(data))
        .catch(e => {
          console.error(e)
          setError(e);
...
export default memo(PMDashboardCampaignMetrics)