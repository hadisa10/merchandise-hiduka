...
import ClientCampaignTimeSales from './pm-campaign-time-sales-activity';
...
function PMCampaignSalesInventoryView({ campaign }: { campaign: ICampaign }) {
    ...
    useEffect(() => {
        if (campaign._id) {
            ...
            setError(null);
            realmApp.currentUser?.functions.searchPMCampaigns(campaign._id.toString()).then((data: ISalesByRegion[]) => setDashboardSalesByRegionMetrics(data))
                .catch(e => {
                    console.error(e)
                    setError(e);
                    enqueueSnackbar("Failed to get dashboard Metrics", { variant: "error" })
...
export default memo(PMCampaignSalesInventoryView)